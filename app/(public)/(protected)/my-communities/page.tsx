...
export default function DashboardPage() {
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            if (userAddress) params.set("user_address", userAddress);
            const res = await fetch(`/api/community/get-owned-communities?${params}`);
            const json = await res.json();
    ...
    return (
        <article className="px-4 pb-16 md:px-8 py-4 space-y-3">
            <h2 className="font-bold">Your Community</h2>
                                ...
                                {filtered.map((community) => (
                                    <CommunityCard key={community.communityID}>
                                        ...
                                        <CommunityCardActions>
                                            ...
                                            <CommunityCardDeleteButton communityId={community.communityID} communityName={community.name} isCreator={community.isCreator} />
                                        </CommunityCardActions>
                                    </CommunityCard>
                                ))}
                            </div>
